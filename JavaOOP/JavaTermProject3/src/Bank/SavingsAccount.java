...
public class SavingsAccount extends Account{
	...
	public boolean debit(int amount){
		...
		if ((currentYear - startYear) >= maturity) {
			...
			JOptionPane.showMessageDialog(null,"만기가 지나 출금이 가능하지만 출금 후 이자는 적용되지 않습니다.","ATM",JOptionPane.INFORMATION_MESSAGE);
			this.isMatured = true;
			return super.debit(amount);
		}
		else {
			//System.out.println("예금 계좌에서는 출금할 수 없습니다.");
			JOptionPane.showMessageDialog(null,"예금 계좌에서는 출금할 수 없습니다.","ATM",JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
	}
	...
	public void applyInterest(int years) {
		if (!isMatured) {
			...
			JOptionPane.showMessageDialog(null,"이율이 적용되었습니다.\n"+"이전 금액: " + previousBalance+"\n"+"현재 금액: " + afterBalance,"ATM",JOptionPane.INFORMATION_MESSAGE);
		}
	}
}